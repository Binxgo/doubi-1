...
Install_Libsodium(){
	...
	if [[ ${release} == "centos" ]]; then
		...
		echo -e "${Info} 解压..."
		tar -xzf libsodium-${Libsodiumr_ver}-RELEASE.tar.gz && cd libsodium-${Libsodiumr_ver}-RELEASE
		echo -e "${Info} 编译安装..."
	...
	ldconfig
	cd .. && rm -rf libsodium-${Libsodiumr_ver}-RELEASE.tar.gz && rm -rf libsodium-${Libsodiumr_ver}-RELEASE
